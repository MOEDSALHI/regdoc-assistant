...
import pytest
from src.rag.query_expansion import hyde_retrieve, retrieve_with_expansion
...
def test_rrf_multi_query_deduplication():
    ...
    def make_chunk(id, text):
        return ChunkRecord(
            id=id, document_id=1, chunk_index=id, text=text,
            token_count=5, chunk_strategy="by_article", page_number=1,
            section_title=None, similarity=0.9, filename="test.txt"
        )
